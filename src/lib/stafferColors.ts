...
function readColorOverrides(
  colorByStaffer?: Map<string, string | null | undefined> | Record<string, string | null | undefined>
) {
  const overrides = new Map<string, string>();
  if (!colorByStaffer) return overrides;

  const entries =
    colorByStaffer instanceof Map ? [...colorByStaffer.entries()] : Object.entries(colorByStaffer);

  for (const [name, color] of entries) {
    if (!color) continue;
    overrides.set(name, normalizeHex(color));
  }
  return overrides;
}

export function buildStafferColorMap(
  stafferNames: string[],
  colorByStaffer?: Map<string, string | null | undefined> | Record<string, string | null | undefined>
) {
  const sorted = [...new Set(stafferNames)].sort((a, b) => a.localeCompare(b));
  const overrides = readColorOverrides(colorByStaffer);
  const reserved = new Set([STAFFER_MAP_UNASSIGNED, ...overrides.values()]);
...
export function stafferColorOverrides(
  staffers: Iterable<{ name: string; map_color?: string | null }>
) {
  const overrides: Record<string, string> = {};
  for (const staffer of staffers) {
    if (staffer.map_color) overrides[staffer.name] = staffer.map_color;
  }
  return overrides;
}

export function buildStafferColorOverrideMap(
  stafferColors: Record<string, string> | undefined,
  countyStaffers: StafferMapEntry[],
  districtStaffers: StafferDistrictEntry[]
) {
  return {
    ...(stafferColors ?? {}),
    ...stafferColorOverrides(countyStaffers),
